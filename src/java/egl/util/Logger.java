...
/**
 * Handle logging for the Eagle node server
 */
public final class Logger {
     ...
     * The egl/conf/logging-default.properties and egl/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "egl.util.EagleLogManager");
        if (!(LogManager.getLogManager() instanceof EagleLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("egl.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Eagle.loadProperties(loggingProperties, "logging-default.properties", true);
                Eagle.loadProperties(loggingProperties, "logging.properties", false);
                Eagle.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(egl.Eagle.class);
        enableStackTraces = Eagle.getBooleanProperty("egl.enableStackTraces");
        enableLogTraceback = Eagle.getBooleanProperty("egl.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof EagleLogManager) {
            ((EagleLogManager) LogManager.getLogManager()).eglShutdown();
        }
    }
