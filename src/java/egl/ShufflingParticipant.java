...
public final class ShufflingParticipant {
    ...
    void setData(byte[][] data, int timestamp) {
        if (data != null && Eagle.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, Eagle.getBlockchain().getHeight()));
        }
    }
