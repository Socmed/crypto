...
import static egl.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static egl.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static egl.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static egl.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = Eagle.getStringProperty("egl.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Eagle.getIntProperty("egl.maxAPIRecords");
    static final boolean enableAPIUPnP = Eagle.getBooleanProperty("egl.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Eagle.getIntProperty("egl.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Eagle.getBooleanProperty("egl.apiServerCORS");
    private static final String forwardedForHeader = Eagle.getStringProperty("egl.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Eagle.getStringListProperty("egl.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Eagle.getStringListProperty("egl.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Eagle.getStringListProperty("egl.allowedBotHosts");
        ...
        boolean enableAPIServer = Eagle.getBooleanProperty("egl.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Eagle.getIntProperty("egl.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Eagle.getIntProperty("egl.apiServerSSLPort");
            final String host = Eagle.getStringProperty("egl.apiServerHost");
            disableAdminPassword = Eagle.getBooleanProperty("egl.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Eagle.getBooleanProperty("egl.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Eagle.getUserHomeDir()).resolve(Paths.get(Eagle.getStringProperty("egl.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Eagle.getStringProperty("egl.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Eagle.getStringProperty("egl.keyStoreType"));
                List<String> ciphers = Eagle.getStringListProperty("egl.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Eagle.getStringProperty("egl.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Eagle.getStringProperty("egl.apiWelcomeFile")});
            }

            String javadocResourceBase = Eagle.getStringProperty("egl.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/egl");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Eagle.getIntProperty("egl.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/egl-proxy");
            ...
            if (!Eagle.getBooleanProperty("egl.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/egl", "/egl-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(egl.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Eagle.getBooleanProperty("egl.apiFrameOptionsSameOrigin")) {
