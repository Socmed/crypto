...
import static egl.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, Eagle.getBlockchain().getHeight(), false);
    }
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.EGL);
        if (holdingType == HoldingType.EGL && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is EGL"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return Eagle.newTransactionBuilder(json);
            } catch (EagleException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return Eagle.newTransactionBuilder(bytes, prunableAttachments);
            } catch (EagleException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, EagleException.NotValidException {
