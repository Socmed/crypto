...
import egl.AccountLedger;
import egl.AccountLedger.LedgerEntry;
import egl.Block;
import egl.BlockchainProcessor;
import egl.Db;
import egl.Eagle;
import egl.Transaction;
import egl.TransactionProcessor;
 ...
 * has not been received within egl.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by egl.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Eagle.getIntProperty("egl.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Eagle.getIntProperty("egl.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Eagle.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Eagle.getTransactionProcessor();
    ...
    /** Eagle event listeners */
    private final List<EagleEventListener> eglEventListeners = new ArrayList<>();
     ...
     *
     * Eagle event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Eagle event registrations
     ...
     *
     * @param   eventRegistrations      Eagle event registrations
     * @throws  EventListenerException  Invalid Eagle event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<EagleEventListener> it = eglEventListeners.iterator();
                while (it.hasNext()) {
                    EagleEventListener listener = it.next();
                ...
                if (addListener) {
                    EagleEventListener listener = new EagleEventListener(event);
                    listener.addListener();
                    eglEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            eglEventListeners.forEach(EagleEventListener::removeListener);
        } finally {
            lock.unlock();
        }
    ...
    /**
     * Eagle event listener
     */
    private class EagleEventListener {

        /** Event handler */
        private final EagleEventHandler eventHandler;

        /**
         * Create the Eagle event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public EagleEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Eagle listener for this event
         */
        ...
        /**
         * Remove the Eagle listener for this event
         */
        ...
        /**
         * Return the hash code for this Eagle event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Eagle events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof EagleEventListener) &&
                    eventHandler.equals(((EagleEventListener)obj).eventHandler));
        }

        /**
         * Eagle listener event handler
         */
        private abstract class EagleEventHandler {
            ...
            /** Eagle listener event */
            protected final Enum<? extends Enum> event;

            /**
             * Create the Eagle event handler
             *
             * @param   eventRegistration   Event registration
             */
            public EagleEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof EagleEventHandler) &&
                                        owner == ((EagleEventHandler)obj).owner &&
                                        accountId == ((EagleEventHandler)obj).accountId &&
                                        event == ((EagleEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends EagleEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends EagleEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends EagleEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends EagleEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Eagle listener event
        ...
        /**
         * Return the Eagle listener event
         *
         * @return                  Eagle listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
