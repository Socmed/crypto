...
import egl.db.VersionedPrunableDbTable;
import egl.db.VersionedValuesDbTable;
...
public class TaggedData {
        ...
        protected void prune() {
            if (Constants.ENABLE_PRUNING) {
                try (Connection con = db.getConnection();
                    ...
                    int expiration = Eagle.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
    ...
    public static final class Tag {
        ...
        private static void add(TaggedData taggedData) {
            for (String tagValue : taggedData.getParsedTags()) {
                ...
                if (tag == null) {
                    tag = new Tag(tagValue, Eagle.getBlockchain().getHeight());
                }
                tag.count += 1;
                tagTable.insert(tag);
            }
    ...
    public TaggedData(Transaction transaction, Attachment.TaggedDataAttachment attachment) {
        this(transaction, attachment, Eagle.getBlockchain().getLastBlockTimestamp(), Eagle.getBlockchain().getHeight());
    }
    ...
    static void add(TransactionImpl transaction, Attachment.TaggedDataUpload attachment) {
        if (Eagle.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME && attachment.getData() != null) {
    ...
    static void extend(Transaction transaction, Attachment.TaggedDataExtend attachment) {
        ...
        if (Eagle.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME < timestamp.timestamp) {
            ...
            if (taggedData != null) {
                ...
                taggedData.blockTimestamp = Eagle.getBlockchain().getLastBlockTimestamp();
                taggedData.height = Eagle.getBlockchain().getHeight();
                taggedDataTable.insert(taggedData);
            }
        }
    }
