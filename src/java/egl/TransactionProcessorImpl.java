...
import egl.db.DbClause;
import egl.db.DbIterator;
import egl.db.DbKey;
import egl.db.EntityDbTable;
...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Eagle.getBooleanProperty("egl.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Eagle.getBooleanProperty("egl.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Eagle.getIntProperty("egl.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Eagle.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Eagle.getEpochTime()), 0, -1, "")) {
    ...
    @Override
    public void broadcast(Transaction transaction) throws EagleException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws EagleException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Eagle.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (EagleException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (EagleException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws EagleException.NotValidException {
        if (Eagle.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (EagleException.NotCurrentlyValidException ignore) {
            } catch (EagleException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new EagleException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws EagleException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Eagle.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new EagleException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new EagleException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new EagleException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Eagle.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new EagleException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new EagleException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new EagleException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new EagleException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    throw new EagleException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new EagleException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws  EagleException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws EagleException.NotValidException {
