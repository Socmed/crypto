...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/www/img/egl-icon-32x32.png", "tray icon");
        ...
        if (!Eagle.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("egldesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("egldesktop.DesktopApplication failed to launch", exception);
            }
        });

        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("egldesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("egldesktop.DesktopApplication failed to refresh", exception);
            }
        });
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", Eagle.APPLICATION);
        addDataRow(statusPanel, "Version", Eagle.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(Eagle.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(Eagle.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", Eagle.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "EGL Server Status");
