 ...
 *
 *   java -cp "classes:lib/*:conf" egl.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Degl.runtime.mode=desktop egl.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Eagle properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "egl.testDb" : "egl.db";
        String dbType = Eagle.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Eagle.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Eagle.getDbDir(Eagle.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Eagle.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Eagle.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Eagle.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "egl.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "egl.mv.db");
            if (!dbFile.exists()) {
