...
import egl.Db;
import egl.FxtDistribution;
import egl.Eagle;
import egl.EagleException;
import egl.http.APIServlet;
import egl.http.APITag;
import egl.http.JSONResponses;
import egl.http.ParameterParser;
...
public final class JPLSnapshot implements AddOn {
     ...
     * requirement to existing EGL holders.</p>
     *
     * <p>This utility takes a snapshot of account balances and public keys on the Eagle blockchain as of the specified height,
     ...
     * <li>height - the Eagle blockchain height at which to take the snapshot</li>
     * </ul>
     *
     * <p>Response</p>
     ...
     * for a total of 900M to new accounts, resulting in 100M automatically allocated to existing EGL holders:
     * <pre>
     * {
     *     "balances": {
     *         "EGL-NZKH-MZRE-2CTT-98NPZ": 30000000000000000,
     *         "EGL-X5JH-TJKJ-DVGC-5T2V8": 30000000000000000,
     *         "EGL-LTR8-GMHB-YG56-4NWSE": 30000000000000000
     *     },
     *     "publicKeys": [
    ...
    public static class JPLSnapshotAPI extends APIServlet.APIRequestHandler {
        ...
        protected JSONStreamAware processRequest(HttpServletRequest request, HttpServletResponse response) throws EagleException {
            ...
            Eagle.getBlockchainProcessor().addListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
            Eagle.getBlockchainProcessor().scan(height - 1, false);
            Eagle.getBlockchainProcessor().removeListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
    ...
    private static class JPLSnapshotListener implements Listener<Block> {
        ...
        public void notify(Block block) {
            if (block.getHeight() == height) {
                ...
                SortedMap<String, Long> snapshotEagleBalances = snapshotEagleBalances();
                BigInteger snapshotTotal = BigInteger.valueOf(snapshotEagleBalances.values().stream().mapToLong(Long::longValue).sum());
                ...
                if (inputBalances != null) {
                    ...
                    if (!inputTotal.equals(BigInteger.ZERO)) {
                        snapshotEagleBalances.entrySet().forEach(entry -> {
                    ...
                    inputBalances.entrySet().forEach(entry -> {
                        ...
                        snapshotEagleBalances.merge(account, inputBalance, (a, b) -> a + b);
                    });
                }
                snapshot.put("balances", snapshotEagleBalances);
            }
        }
        ...
        private SortedMap<String, Long> snapshotEagleBalances() {
