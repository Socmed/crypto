    ...
    public void init() {
        ...
        Eagle.getBlockchainProcessor().addListener(new Listener<Block>() {
            ...
            public void notify(Block block) {
                if (block.getHeight() == Eagle.getHardForkHeight()) {
                    exportPublicKeys();
            ...
            private Map<String, Long> exportIgnisBalances() {
                ...
                try (Connection con = Db.db.getConnection();
                    ...
                    try (ResultSet rs = pstmt.executeQuery()) {
                        while (rs.next()) {
                            ...
                            if (Constants.isTestnet) {
                                btcSnapshotMap.put(account, (balance / Constants.ONE_EGL) * 600);
                                usdSnapshotMap.put(account, ((balance * 300 / 5) / Constants.ONE_EGL));
                                eurSnapshotMap.put(account, ((balance * 300 / 5) / Constants.ONE_EGL));
                            }
                        }
                    }
                } catch (SQLException e) {
                ...
                try (DbIterator<? extends Transaction> transactions = Eagle.getBlockchain().getTransactions(
                        FxtDistribution.FXT_ISSUER_ID, 0, TransactionType.Payment.ORDINARY.getType(),
                        TransactionType.Payment.ORDINARY.getSubtype(), Eagle.getBlockchain().getBlockAtHeight(FxtDistribution.DISTRIBUTION_START).getTimestamp(),
                ...
                if (Constants.isTestnet && !developerPublicKeys.isEmpty()) {
                    ...
                    developerPublicKeys.forEach(publicKey -> {
                        ...
                        btcSnapshotMap.put(account, (developerBalance / Constants.ONE_EGL) * 600);
                        usdSnapshotMap.put(account, ((developerBalance * 300 / 5) / Constants.ONE_EGL));
                        eurSnapshotMap.put(account, ((developerBalance * 300 / 5) / Constants.ONE_EGL));
                    });
                }
