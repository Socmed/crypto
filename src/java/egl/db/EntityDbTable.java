...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Eagle.getBlockchainProcessor().isScanning() ?
                    Math.max(Eagle.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return Eagle.getBlockchain().getHeight() <= height && ! (isPersistent() && Eagle.getBlockchainProcessor().isScanning());
    }
