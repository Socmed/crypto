...
import egl.EagleException.AccountControlException;
...
public final class AccountRestrictions {
    ...
    public static final class PhasingOnly {
        ...
        private void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws AccountControlException {
            if (!validatingAtFinish && maxFees > 0 && Math.addExact(transaction.getFeeNQT(), PhasingPoll.getSenderPhasedTransactionFees(transaction.getSenderId())) > maxFees) {
                throw new AccountControlException(String.format("Maximum total fees limit of %f EGL exceeded", ((double)maxFees)/Constants.ONE_EGL));
            }
            ...
            if (!validatingAtFinish) {
                int duration = phasingAppendix.getFinishHeight() - Eagle.getBlockchain().getHeight();
    ...
    static void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws EagleException.NotCurrentlyValidException {
        Account senderAccount = Account.getAccount(transaction.getSenderId());
        if (senderAccount == null) {
            throw new EagleException.NotCurrentlyValidException("Account " + Long.toUnsignedString(transaction.getSenderId()) + " does not exist yet");
        }
