...
import egl.util.CountingInputReader;
import egl.util.CountingInputStream;
import egl.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Eagle.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Eagle.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Eagle.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_EGL) / Constants.MAX_BALANCE_EGL);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof EagleException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    public void blacklist(String cause) {
        blacklistingTime = Eagle.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/egl"));
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new EagleException.EagleIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/egl");
                if (communicationLoggingMask != 0)
        ...
        } catch (EagleException.EagleIOException e) {
            blacklist(e);
            if (connection != null) {
                connection.disconnect();
            }
    ...
    void connect() {
        lastConnectAttempt = Eagle.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_EGL * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
