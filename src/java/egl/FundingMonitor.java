 ...
 * <p>
 * EGL, ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = Eagle.getIntProperty("egl.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for EGL monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.EGL ? holdingId : 0);
        ...
        synchronized(monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.EGL || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Eagle.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Eagle.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case EGL:
                                        processEagleEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
     ...
     * @throws  EagleException                Unable to create transaction
     */
    private static void processEagleEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = Eagle.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short)1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(Eagle.getBlockchain().getLastBlockTimestamp());
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                monitoredAccount.height = Eagle.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("EGL funding transaction %s for %f EGL submitted from %s to %s",
                        transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_EGL,
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            //
            // Check the EGL balance for monitored accounts
            //
            synchronized(monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                       if (maccount.monitor.holdingType == HoldingType.EGL && balance < maccount.threshold &&
