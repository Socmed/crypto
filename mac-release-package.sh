...
then
echo obfuscate
~/proguard/proguard5.3.3/bin/proguard.sh @egl.pro
...
/Library/Java/JavaVirtualMachines/jdk1.8.0_121.jdk/Contents/Home/bin/javapackager -deploy -outdir . -outfile egl-client -name egl-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.egl.client.installer -Bmac.CFBundleName=Eagle-Installer -Bmac.CFBundleVersion=${MACVERSION} -BappVersion=${MACVERSION} -Bicon=installer/AppIcon.icns -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting EGL (YU63QW5EFW)" > installer/javapackager.log 2>&1
