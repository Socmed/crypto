Building the EGL installer

Note: all pathes are relative to the git repository folder (egl-private)
...
Test X11 installer: java -jar egl-client-<version>.jar
Test console installer: java -jar egl-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your egl-private git repository preserving the folder structure
...
Test the installer using Java: java -jar egl-client-<version>.jar
Test the windows executable: egl-client-<version>.exe
...
egl.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
