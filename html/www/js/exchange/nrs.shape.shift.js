...
var NRS = (function(NRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "EGL_" + coin : coin + "_EGL";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("EGL_") == 0) {
            return pair.substring("EGL_".length);
        }
        if (pair.indexOf("_EGL") == pair.length - "_EGL".length) {
            return pair.substring(0, pair.indexOf("_EGL"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_egl");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "EGL" && String(transaction.curOut).escapeHTML() != "EGL") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderEagleLimit() {
        apiCall('limit/egl_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_egl_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'EGL' && coin['status'] == 'available') {
    ...
    NRS.getFundAccountLink = function() {
        return "<div class='callout callout-danger'>" +
            ...
            "data-pair='BTC_EGL'>" + $.t("fund_account_message") + "</a>";
    };
    ...
    $("#shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#shape_shift_buy_title").html($.t("exchange_egl_to_coin_shift", { coin: coin }));
        $("#shape_shift_buy_min").val(invoker.data("min"));
        $("#shape_shift_buy_min_coin").html("EGL");
        $("#shape_shift_buy_max").val(invoker.data("max"));
        $("#shape_shift_buy_max_coin").html("EGL");
        $("#shape_shift_buy_rate").val(invoker.data("rate"));
        $("#shape_shift_buy_rate_text").html("EGL/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_egl_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("EGL/" + coin);
    ...
    $('#m_send_amount_buy_withdrawal_amount, #m_send_amount_buy_withdrawal_address').change(function () {
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                // add 1 EGL fee to make sure the net amount is what requested by shape shift
    ...
    $("#shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#shape_shift_sell_title").html($.t("exchange_coin_to_egl_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#shape_shift_sell_rate_text").html(coin + "/EGL");
                $("#shape_shift_sell_fee").val(data.fee);
                $("#shape_shift_sell_fee_coin").html("EGL");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_egl_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("EGL/" + coin);
        $("#m_send_amount_sell_fee_coin").html("EGL");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("EGL");
