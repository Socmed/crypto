...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.sellAlias = function ($modal) {
        ...
        if (data.modal == "cancel_alias_sale") {
            ...
            if (data.recipient == NRS.constants.GENESIS_RS) {
                if (!data.priceEGL || data.priceEGL == "0") {
                    return {
                        "error": $.t("error_not_specified", {
        ...
        if (data.priceNQT == "0") {
            ...
            $.growl(
                $.t("success_alias_sell", {
                    ...
                    "price": NRS.convertToEGL(data.priceNQT).escapeHTML()
                }), {
                    "type": "success"
                }
            );
        ...
        NRS.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                if (!("priceNQT" in response)) {
                    ...
                    $modal.find("input[name=amountEGL]").val(NRS.convertToEGL(response.priceNQT)).prop("readonly", true);
                }
            }
        }, { isAsync: false });
    });
    ...
    NRS.forms.buyAliasError = function () {
        $("#buy_alias_modal").find("input[name=priceEGL]").prop("readonly", false);
    };
    ...
    NRS.forms.buyAliasComplete = function (response, data) {
        ...
        $.growl(
            $.t("success_alias_buy", {
                "alias_name": data.aliasName,
                "price": NRS.convertToEGL(data.amountNQT).escapeHTML()
            }), {
                "type": "success"
            }
        );
        ...
        if (alias) {
            ...
            NRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    } else if ((aliasURI = /acct:(.*)@egl/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    NRS.forms.setAlias = function ($modal) {
        ...
        if (data.type == "account") {
            if (!(/acct:(.*)@egl/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(EGL\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@egl";
                    }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            registerAliasUri.val("").mask("EGL-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@egl/i);
                if (!match) {
                ...
                } else if (!/^EGL\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = NRS.accountRS;
                }
                    ...
                    if ("priceNQT" in response) {
                        if (response.buyer == NRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "egl": NRS.formatAmount(response.priceNQT)
