...
jar/exe/dmg package, or run "java -jar egl-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/egl/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Eagle server.


Using the egl-client.zip package:

Unpack the egl-client.zip package and open a shell in the resulting egl
...
the egl_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.egl for
configuration files, logs, and the egl_db database. Unlike run.sh, start.sh
...
message "Eagle server 1.x.x started successfully" in the console log. If run in
...
where the Eagle UI should now be available.
...
to the Eagle installation directory, as the use of special characters may result
...
conf/egl-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
egl-default.properties directly, but create a new conf/egl.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from egl-default.properties, the
settings in egl.properties override those in egl-default.properties. This way,
when upgrading the software, you can safely overwrite egl-default.properties
with the updated file from the new package, while your customizations remain
safe in the egl.properties file.


How to contribute?

There are many ways to contribute to Eagle. Here are some examples:
...
The Eagle software is a client-server application. It consists of a java server
...
requests which other Eagle client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
egl_db directory. When upgrading, you should not delete the old egl_db
...
egl_db, except that it will take some extra time to download the blockchain
from scratch.

The default Eagle client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file egl.log, which gets
overwritten at restart. In case of an error, the egl.log file may contain
...
egl.jar file using the enclosed jar.sh script, and then egl.jar should be
