...
import egl.VoteWeighting.MinBalanceModel;
import egl.VoteWeighting.VotingModel;
import egl.http.APICall;
import egl.http.APICall.Builder;
...
public class PhasingOnlyTest extends BlockchainTest {
    ...
    public void testAccountVoting() throws Exception {
        ...
        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
            .recipient(BOB.getId())
            .param("amountNQT", 1 * Constants.ONE_EGL);
    ...
    public void testExtraRestrictions() throws Exception {
        //all transactions must be approved either by BOB or CHUCK, total fees 5 EGL, min duration 4, max duration 100
        setPhasingOnlyControl(VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()},
                5 * Constants.ONE_EGL, 4, 100);

        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
                .recipient(BOB.getId())
                .param("amountNQT", 1 * Constants.ONE_EGL)
                .feeNQT(7 * Constants.ONE_EGL);
        // fee too high
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);

        // fee at the limit
        builder.feeNQT(5 * Constants.ONE_EGL);
        ...
        builder.param("phasingFinishHeight", Eagle.getBlockchain().getHeight() + 200);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", Eagle.getBlockchain().getHeight() + 3);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", Eagle.getBlockchain().getHeight() + 4);
        ACTestUtils.assertTransactionSuccess(builder);
    ...
    public void testRejectingPendingTransaction() throws Exception {
        ...
        Assert.assertEquals(balanceBeforeTransactionRejection + 1 * Constants.ONE_EGL, 
                ACTestUtils.getAccountBalance(ALICE.getId(), "unconfirmedBalanceNQT"));
    }
    
    @Test
    public void testBalanceVoting() {
        setPhasingOnlyControl(VotingModel.NQT, null, 100 * Constants.ONE_EGL, null, null, null, 0, 0, 0);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_EGL, null, null, new long[] {DAVE.getId()});
        ACTestUtils.assertTransactionBlocked(builder);
        
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);
        
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_EGL + 1, null, null, null);
        ACTestUtils.assertTransactionBlocked(builder);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_EGL, null, null, null);
        ACTestUtils.assertTransactionSuccess(builder);
    }
    ...
    private void setTransactionPhasingParams(Builder builder, int finishAfter, VotingModel votingModel, String holdingId, Long quorum,
        ...
        builder.param("phasingFinishHeight", Eagle.getBlockchain().getHeight() + finishAfter);
